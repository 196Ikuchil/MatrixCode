...
public class AbilityList2_importer : AssetPostprocessor {
	private static readonly string filePath = "Assets/Resources/Data/AbilityList2.xlsx";
	private static readonly string exportPath = "Assets/Resources/Data/AbilityList2.asset";
		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(11); p.range = (float)(cell == null ? 0 : cell.NumericCellValue);
						s.list.Add (p);
					}
					data.sheets.Add(s);
				}
